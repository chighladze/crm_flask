...
def update_task(task_id):
    ...
    try:
        ...
        if status_id == '3' and task.task_type_id == 4:
            # Create a subtask with necessary fields
            new_task = Tasks(
                task_type_id=5,
                description=f"ავტომატური შექმნილი დავალება {task.id}",
...
@tasks.route('/tasks/researched_task_orders', methods=['GET'])
@login_required
def researched_task_orders():
    # Предполагается, что у вас есть модели TaskType и TaskStatus
    task_types = TaskTypes.query.all()
    task_statuses = TaskStatuses.query.all()
    return render_template(
        'tasks/researched_task_orders.html',
        task_types=task_types,
        task_statuses=task_statuses,
        active_menu='project_design'
...
@tasks.route('/api/researched_task_orders_list', methods=['GET'])
@login_required
def api_researched_task_orders_list():
    """
    Возвращает данные задач и заказов в формате JSON для отображения в таблице.
    Поддерживает фильтрацию и пагинацию.
    """
...
@tasks.route('/tasks/export_researched_task_orders', methods=['GET'])
@login_required
def export_researched_task_orders():
    """
    Экспортирует фильтрованные данные задач и заказов в файл Excel.
    """
    # Получение параметров фильтрации из запроса
...
@tasks.route('/tasks/research_orders_tasks', methods=['GET'])
@login_required
def research_orders_tasks():
    task_types = TaskTypes.query.all()
    task_statuses = TaskStatuses.query.all()
    return render_template(
...
@tasks.route('/api/research_orders_tasks_list', methods=['GET'])
@login_required
def api_research_orders_tasks_list():
    """
    Возвращает данные задач и заказов в формате JSON для отображения в таблице.
    Поддерживает фильтрацию и пагинацию.
    """
...
@tasks.route('/tasks/export_research_orders_tasks', methods=['GET'])
@login_required
def export_research_orders_tasks():
    """
    Экспортирует фильтрованные данные задач и заказов в файл Excel.
    """
    # Получение параметров фильтрации из запроса
...
@tasks.route('/tasks/rated_orders_tasks', methods=['GET'])
@login_required
def rated_orders_tasks():
    task_types = TaskTypes.query.all()
    task_statuses = TaskStatuses.query.all()
    return render_template(
        'tasks/rated_orders.html',
        task_types=task_types,
        task_statuses=task_statuses,
        active_menu='commercial'

    )


@tasks.route('/api/rated_orders_tasks_list', methods=['GET'])
@login_required
def api_rated_orders_tasks_list():
    """
    Возвращает данные задач и заказов в формате JSON для отображения в таблице.
    Поддерживает фильтрацию и пагинацию.
    """
...
@tasks.route('/tasks/export_rated_orders_tasks', methods=['GET'])
@login_required
def export_rated_orders_tasks():
    """
    Экспортирует фильтрованные данные задач и заказов в файл Excel.
    """
    # Получение параметров фильтрации из запроса
