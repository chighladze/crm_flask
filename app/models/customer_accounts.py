...
class CustomerAccount(db.Model):
    ...
    customer_id = db.Column(db.Integer, db.ForeignKey('customers.id', ondelete='CASCADE'), nullable=False,
                            comment='ID of the customer from the customers table')
    ...
    tariff_plan_id = db.Column(db.Integer, db.ForeignKey('tariff_plans.id', ondelete='CASCADE'), nullable=False,
                               comment='ID of the tariff plan')
    device_name = db.Column(db.String(100), nullable=False, comment='Name of the device (e.g., TP-Link Archer C6)')
    device_type = db.Column(db.Enum('Router', 'Modem', 'ONU', 'STB', 'Other'), nullable=False,
                            comment='Type of the device')
    status = db.Column(db.Enum('Active', 'Inactive', 'Suspended'), default='Active', nullable=True,
                       comment='Account status')
    order_id = db.Column(db.Integer, db.ForeignKey('orders.id', ondelete='SET NULL'), nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False,
                           comment='Date and time of account creation')
